...
class iOSCable: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ref.child("Cables/iOSCables").observe(.childAdded, with: { (snapshot) in
        ...
        ref.child("Cables/iOSCables").observe(.childChanged, with: { (snapshot) in
            if let item = snapshot.value as? [String: String] {
                if let borrower = item["borrower"] {
    ...
    @IBAction func onClikcAdd(_ sender: Any) {
        ...
        let action = UIAlertAction(title: "Add", style: .default) { (alertAction) in
            ...
            if let name = textField.text {
                if name.trimmingCharacters(in: .whitespacesAndNewlines) != "" {
                    FirebaseHandler.addItem(type: "Cables", model: "iOSCables", name: name.trimmingCharacters(in: .whitespacesAndNewlines))
                }
            }
        }
...
extension iOSCable: UITableViewDataSource, UITableViewDelegate {
