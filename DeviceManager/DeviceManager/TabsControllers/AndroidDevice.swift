...
class AndroidDevice: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ref.child("Devices/Android").observe(.childAdded, with: { (snapshot) in
        ...
        ref.child("Devices/Android").observe(.childChanged, with: { (snapshot) in
            if let item = snapshot.value as? [String: String] {
                if let borrower = item["borrower"] {
        ...
        let action = UIAlertAction(title: "Add", style: .default) { (alertAction) in
            ...
            if let name = textField.text {
                if name.trimmingCharacters(in: .whitespacesAndNewlines) != "" {
                    FirebaseHandler.addItem(type: "Devices", model: "Android", name: name.trimmingCharacters(in: .whitespacesAndNewlines))
                }
            }
        }
...
extension AndroidDevice: UITableViewDataSource, UITableViewDelegate {
